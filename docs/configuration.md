...
SciAgent uses a secondary fast model for content processing tasks like web page extraction and document summarization. This model is optimized for speed and cost. Configure it in `src/sciagent/defaults.py`:
...
configure_cache(cache_type="local")

# Or use Redis (assuming a local Redis server running)
configure_cache(cache_type="redis", redis_host="localhost")
