...
- **Keep solver stdout streaming — don't silence it.** `wait_for_job`'s 60-second heartbeat surfaces the last stdout line to the human user, so they can see the run is alive without polling. `solver > log.txt 2>&1` (or anything that redirects stdout to a file) leaves stdout empty — the heartbeat then shows the same stale line forever and the user has no signal whether the solver is progressing or wedged. If you want a log file, use `solver 2>&1 | tee log.txt`: `tee` mirrors to stdout, so the heartbeat keeps working AND you get the file. The agent's own log fetching (`compute_cluster(action="logs", ...)`) works either way; this rule is about the human in the terminal, not the LLM.
